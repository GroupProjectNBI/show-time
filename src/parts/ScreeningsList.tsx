...
import { Clock, MapPin, Film, Ticket, ChevronRight, Calendar } from "lucide-react";
...
export default function ScreeningsList({ screenings }: Props) {
  ...
  const getDisplayDate = (dateString: string) => {
    ...
    // Nollställ tiden för jämförelse av bara datum
    const sDate = new Date(screeningDate).setHours(0, 0, 0, 0);
    const tDate = new Date(today).setHours(0, 0, 0, 0);
    const tmrDate = new Date(tomorrow).setHours(0, 0, 0, 0);

    if (sDate === tDate) return "IDAG";
    if (sDate === tmrDate) return "IMORGON";

    const str = screeningDate.toLocaleDateString('sv-SE', {
      weekday: 'short',
      day: 'numeric',
      month: 'short'
    });
      ...
      {screenings.length === 0 ? (
        ...
        <div className="divide-y divide-white/5">
          {screenings.map((s, index) => {
            // Kolla om vi ska visa en datum-header (om detta är första filmen för en ny dag)
            const currentDate = new Date(s.startTime).toDateString();
            const prevDate = index > 0 ? new Date(screenings[index - 1].startTime).toDateString() : null;
            const showDateHeader = currentDate !== prevDate;

            return (
              <div key={s.id}>
                {/* --- DATUM-HEADER (Visas bara när datumet ändras) --- */}
                {showDateHeader && (
                  <div className="bg-white/5 px-4 py-2 mt-6 first:mt-0 flex items-center gap-2 border-l-2 border-[#c0a060]">
                    <Calendar size={14} className="text-[#c0a060]" />
                    <span className="text-[11px] font-black tracking-[0.2em] text-[#c0a060] uppercase">
                      {getDisplayDate(s.startTime)}
                    </span>
                  </div>
                )}
                    ...
                    <div className="flex flex-col items-end gap-5 pl-8 border-l border-white/5">
                      <div className="flex flex-col items-end gap-1.5">
                        ...
                        <div className="flex items-center gap-2 text-[#c0a060] font-black">
                          ...
                          <span className="text-2xl tracking-tighter uppercase">KL. {formatTime(s.startTime)}</span>
                        </div>
